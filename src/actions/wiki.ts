import { CharacterListStateType, CharacterType } from "@constants/types/wiki";
import {
...
// 更新character页面数据
export const updateWikiCharacter =
  (character: CharacterType) => async (dispatch) => {
    dispatch({
...
export const updateCharacterList_byLocation =
  (name: string, url: string) => async (dispatch) => {
    // 先渲染name
    dispatch({
      type: UPDATE_WIKI_CHARACTER_LIST,
      payload: {
